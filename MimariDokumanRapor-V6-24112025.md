...
### 5.1. Sistem Bağlam Diyagramı (System Context Diagram)
Bu diyagram, sistemin (Web Uygulaması) dış dünya ile olan ilişkisini, kullanıcılarını ve etkileşimde bulunduğu diğer sistemleri en üst seviyeden gösterir.
...
### 5.2. Genel Sistem Mimarisi (Component-Based View)
```mermaid
graph TD
...
### 5.3. Backend Katmanlı Mimarisi (Backend Layered Architecture)
```mermaid
graph TD
