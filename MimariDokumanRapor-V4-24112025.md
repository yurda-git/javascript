...
Aşağıdaki metin tabanlı diyagramlar, bilinen tüm platformlarla (GitHub, GitLab) uyumlu, en basit Mermaid sözdizimi kullanılarak oluşturulmuştur.
...
### 6.2. Referans Dağıtım Modeli (AWS için) (Reference Deployment Model for AWS)
Bu model, bir projenin AWS bulut altyapısı üzerinde standart olarak nasıl dağıtılması gerektiğini ve ana altyapı bileşenlerinin konumlandırılmasını göstermektedir.
...
graph TD
    subgraph "AWS Cloud (eu-central-1)"
        subgraph "VPC (Virtual Private Cloud)"
            ...
            subgraph "Private Subnet"
                BackendASG("Auto Scaling Group Backend API (EC2 Instances)")
                DB_RDS("RDS for PostgreSQL (Multi-AZ)")
            end
            subgraph "Public Subnet"
                 S3_Bucket("S3 Bucket (Static Frontend Files)")
    ...
    CloudFront -- "Static Content" --> S3_Bucket
    CloudFront -- "API Requests (/api/*)" --> LoadBalancer
    LoadBalancer -- "Traffic" --> BackendASG
    BackendASG -- "Data Access" --> DB_RDS
```
